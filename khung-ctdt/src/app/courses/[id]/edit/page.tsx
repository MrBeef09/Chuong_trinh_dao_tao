...
export default function EditCoursePage({ params }: EditCoursePageProps) {
    ...
    const fetchData = async () => {
        try {
            const [majorsResponse, courseResponse] = await Promise.all([
                fetch('/api/majors'),
                fetch(`/api/courses/${params.id}`)
            ])

            if (!majorsResponse.ok || !courseResponse.ok) {
                throw new Error('Failed to fetch data')
            }

            const [majorsData, courseData] = await Promise.all([
                majorsResponse.json(),
                courseResponse.json()
            ])
