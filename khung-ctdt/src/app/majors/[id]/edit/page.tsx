...
export default function EditMajorPage({ params }: EditMajorPageProps) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch faculties and major data in parallel
                const [facultiesResponse, majorResponse] = await Promise.all([
                    fetch('/api/faculties'),
                ...
                if (majorResponse.ok) {
                    const major: Major = await majorResponse.json()
                    setFormData({
                        name: major.name,
                        code: major.code,
                        description: major.description || '',
                        duration: major.duration?.toString() || '',
