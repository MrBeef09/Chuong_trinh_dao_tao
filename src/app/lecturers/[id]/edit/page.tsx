...
export default function EditLecturerPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                // Fetch lecturer data
                const lecturerResponse = await fetch(`/api/lecturers/${params.id}`)
                if (lecturerResponse.ok) {
                    const lecturerData = await lecturerResponse.json()
                    setFormData({
                        ...lecturerData,
                        joinDate: lecturerData.joinDate ? new Date(lecturerData.joinDate).toISOString().split('T')[0] : ''
                    })
                } else {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.error('Failed to update lecturer')
            }
        } catch (error) {
            ...
            <main className="flex-1 lg:ml-0 overflow-auto">
                <div className="p-8">
                    ...
                    <div className="flex items-center mb-8">
                        ...
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900">Edit Lecturer</h1>
